use lol_html::{element, HtmlRewriter, Settings, ElementContentHandlers};

use super::{Engine, Ticket};
use crate::prelude::*;

pub struct Rewriters<'h, 's> {
    settings: Vec<Settings<'h, 's>>
}

impl <'h, 's> Rewriters<'h, 's> {
    pub fn push_setting(&mut self, settings: Settings<'h, 's>) {
        self.settings.push(settings);
    }
    
    pub fn rewrite(self, source: String) -> Result<String> {
        let mut output_buff: Vec<u8> = Vec::with_capacity(source.len());
        let mut input_buff: Vec<u8> = source.into();

        for settings in self.settings {
            let mut rewriter = HtmlRewriter::new(
                settings,
                |bytes: &[u8]| output_buff.extend_from_slice(bytes)
            );
            rewriter.write(&input_buff)?;

            input_buff.clear();
            input_buff.append(&mut output_buff);
        }

        String::from_utf8(output_buff)
            .context("Invalid UTF-8 generated by rewriter.")
    }
}
