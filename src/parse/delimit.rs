...
impl<'a> Delimiters<'a> {
    ...
    pub fn parse_into<T>(&self, source: &'a str) -> Result<Vec<T>> where T: TryFrom<Delimited<'a>, Error=Report> {
        self.parse_iter(source)
            .map(|d| T::try_from(d) )
            .collect()
    }
    ...
    pub fn expand(&self, source: &mut String, mut replacer: impl FnMut(Delimited) -> Result<String>) -> Result<()> {
...
mod raw {
    ...
    fn complex() {
        ...
        let source = "Here is a block shortcode:\n{% sc code %}\nprintln!(\"Hello, world!\");\n{% endsc %}";

        let del = Delimiters::new("{% sc ", "{% endsc %}", DelimiterKind::Raw);
        let code = &del.parse_from(source)[0];

        assert_eq!(code.contents, "{% sc code %}\nprintln!(\"Hello, world!\");\n{% endsc %}")
    }
}